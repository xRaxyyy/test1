...
function initializeCart() {
    ...
    if (userId) {
        ...
        storedCart = localStorage.getItem('guestCart1');
    }
    ...
    fetch('products.json')
        ...
        .then(products => {
            ...
            if (userId) {
                ...
                localStorage.setItem('guestCart1', JSON.stringify(cart)); // Save to session storage for guests
            }
...
function removeFromCart(itemName) {
    ...
    const cartKey = userId ? 'accountCart1' : 'guestCart1';
